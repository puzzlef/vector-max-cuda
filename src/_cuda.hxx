...
template <class T>
__device__ size_t countKernelLoop(const T *x, T v, size_t N, size_t i, size_t DI) {
  ASSERT(x && DI);
  size_t a = 0;
  for (; i<N; i+=DI)
    if (x[i]==v) ++a;
...
__global__ void countKernelW(TA *a, const T *x, T v, size_t N) {
  DEFINE(t, b, B, G);
  ASSERT(a && x);
  __shared__ TA cache[S];
  cache[t] = countKernelLoop(x, v, N, B*b+t, G*B);
  sumKernelReduceU(cache, B, t);
  if (t==0) a[b] = cache[0];
}

template <class T, class TA>
void countMemcpyCuW(TA *a, const T *x, T v, size_t N) {
  ASSERT(a && x);
  const int B = BLOCK_SIZE(N,  BLOCK_LIMIT_REDUCE);
  const int G = GRID_SIZE(N, B, GRID_LIMIT_REDUCE);
  countKernelW<<<G, B>>>(a, x, v, N);
  ASSERT_KERNEL();
}

template <class T, class TA>
void countInplaceCuW(TA *a, const T *x, T v, size_t N) {
  ASSERT(a && x);
  const int B = BLOCK_SIZE(N,  BLOCK_LIMIT_REDUCE);
  const int G = GRID_SIZE(N, B, GRID_LIMIT_REDUCE);
  countKernelW<<<G, B>>>(a, x, v, N);
  ASSERT_KERNEL();
  countKernelW<<<1, G>>>(a, a, v, G);
  ASSERT_KERNEL();
}

template <class T, class TA>
void countCuW(TA *a, const T *x, T v, size_t N) {
  ASSERT(a && x);
  countMemcpyCuW(a, x, v, N);
}
...
template <class T>
__device__ size_t countNonZeroKernelLoop(const T *x, size_t N, size_t i, size_t DI) {
  ASSERT(x && DI);
  size_t a = 0;
  for (; i<N; i+=DI)
    if (x[i]) ++a;
...
__global__ void countNonZeroKernelW(TA *a, const T *x, size_t N) {
  ...
  ASSERT(a && x);
  __shared__ TA cache[S];
  cache[t] = countNonZeroKernelLoop(x, N, B*b+t, G*B);
  sumKernelReduceU(cache, B, t);
  if (t==0) a[b] = cache[0];
}
...
template <class T>
void exclusiveScanW(T *a, const T *x, size_t N) {
  ASSERT(a && x);
...
template <class T>
void inclusiveScanW(T *a, const T *x, size_t N) {
  ASSERT(a && x);
  thrust::device_ptr<T> xD((T*) x), aD(a);
